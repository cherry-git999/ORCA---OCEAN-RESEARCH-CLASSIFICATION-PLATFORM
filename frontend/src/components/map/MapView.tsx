...
import { Compass, AlertTriangle, Navigation } from 'lucide-react';
...
export const MapView: React.FC<MapViewProps> = ({
  ...
  useEffect(() => {
    ...
    if (!mapInstanceRef.current) {
      ...
      L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png', {
        attribution: '&copy; OpenStreetMap contributors &copy; CARTO (Offshore Marine Basemap)',
        subdomains: 'abcd',
        maxZoom: 19,
      }).addTo(map);
  ...
  // 2. Synchronize Center View when Coordinates Change
  useEffect(() => {
    if (mapInstanceRef.current) {
  ...
  // 3. Render Transect Swath & Anomaly Waypoint Markers
  useEffect(() => {
    ...
    detections.forEach((det, idx) => {
      ...
      marker.bindPopup(
        ...
        <div style="font-family: var(--font-body, sans-serif); padding: 4px; min-width: 180px;">
          ...
          <div style="font-size: 10px; color: #cbd5e1; margin-top: 6px; font-family: monospace;">
            Lat: ${markerLat > 0 ? markerLat.toFixed(4) + '° N' : Math.abs(markerLat).toFixed(4) + '° S'}
            <br />
            Lon: ${markerLng > 0 ? markerLng.toFixed(4) + '° E' : Math.abs(markerLng).toFixed(4) + '° W'}
          </div>
          <div style="font-size: 9px; color: #fbbf24; margin-top: 4px; font-weight: 600;">
            ${ACCURACY_DISCLAIMER}
          </div>
        </div>
      `,
  ...
  }, [detections, centerLat, centerLng, imageWidth, imageHeight, onSelectAnomaly]);

  // 4. Synchronize Selected Anomaly Popup
  useEffect(() => {
