...
import { Cpu, CheckCircle2, ShieldCheck, Layers } from 'lucide-react';
    ...
    <div className="glass-panel" style={{ padding: '20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        ...
        <span className="badge badge-emerald">
          ...
          2/2 Models Operational
        </span>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '14px' }}>
        {models.map((model) => {
          const isCurrentlyActive = activeModelName?.includes(model.target);
          return (
            <div
              key={model.id}
              style={{
            ...
            >
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}>
                  <span style={{ color: 'var(--text-muted)' }}>Baseline Dataset:</span>
      ...
      <div style={{
        ...
        <span>
          Target-aware routing ensures inference is delegated strictly to the validated specialist model. Unsupported semantic classes are rejected by policy.
        </span>
      </div>
    </div>
  );
