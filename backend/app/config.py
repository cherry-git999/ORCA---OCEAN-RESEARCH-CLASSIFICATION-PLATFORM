...
DEFAULT_MODELS_DIR: Path = REPO_ROOT / "models"
MODELS_DIR: Path = Path(os.getenv("ORCA_MODEL_DIR", str(DEFAULT_MODELS_DIR))).resolve()
...
class Settings:
    ...
    # LEGACY LOCAL FALLBACK (Reference only — do not use in portable runtime):
    ...
    MODEL_1_PATH: Path = MODELS_DIR / "pipeline" / "best.pt"
    ...
    MODEL_2_PATH: Path = MODELS_DIR / "human" / "best.pt"
    ...
    MODEL_3_PATH: Path = MODELS_DIR / "hardware" / "best.pt"
